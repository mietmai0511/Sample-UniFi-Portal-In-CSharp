...
namespace UnfiConnect
{
    ...
    public class Api
    {
        ...
        public bool AuthenticateIsSync(string username, string password)
        {
            // Create a new cookie container to contain the authentication cookie
            _cookieContainer = new CookieContainer();
            ...
            // Send an authentication request
            var authUri = new Uri(BaseUri, "/api/login");
            var resultString = HttpUtility.AuthenticateViaJsonPostMethodIsSync(authUri, username, password, _cookieContainer, ConnectionTimeout);
            var resultJson = JsonConvert.DeserializeObject<Responses.ResponseEnvelope<Responses.BaseResponse>>(resultString);
        ...
        public Responses.ResponseEnvelope<Responses.Clients> AuthorizeGuestIsSync(string macAddress)
        {
            // Make the POST request towards the UniFi API to request authorizing the client with the provided MAC address
            var resultString = HttpUtility.PostRequestIsSync(new Uri(BaseUri, $"/api/s/{SiteId}/cmd/stamgr"),
                "{\"mac\":\"" + macAddress + "\",\"cmd\":\"authorize-guest\"}",
                _cookieContainer,
                ConnectionTimeout);
            var resultJson = JsonConvert.DeserializeObject<Responses.ResponseEnvelope<Responses.Clients>>(resultString);
        ...
        public bool LogoutIsSync()
        {
            // Create a session towards the UniFi Controller
            var logoutUri = new Uri(BaseUri, "/api/logout");
            var resultString = HttpUtility.LogoutViaJsonPostMethodIsSync(logoutUri, _cookieContainer, ConnectionTimeout);
            var resultJson = JsonConvert.DeserializeObject<Responses.ResponseEnvelope<Responses.BaseResponse>>(resultString);
